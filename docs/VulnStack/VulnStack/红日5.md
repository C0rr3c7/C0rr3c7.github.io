...
```
# Nmap 7.94SVN scan initiated Fri Apr 17 04:54:43 2026 as: nmap -sT -T4 -Pn -p- -oN ports 192.168.111.150
...
包括端口扫描以发现开放服务，Web应用信息探测（如识别ThinkPHP框架及版本）。

### 2. Web攻防与漏洞利用

ThinkPHP框架相关漏洞（如5.0.22/5.1.29 RCE、5.0.23 RCE、文件包含漏洞、数据库信息泄露等）的检测与利用，实现命令执行和获取Webshell。

### 3. 权限提升

利用系统漏洞（如ms14-058）进行本地提权，获取更高权限。

### 4. 内网渗透

内网信息收集（如判断是否存在域、查看域控、扫描内网存活主机及开放端口），抓取用户明文密码和哈希值。

### 5. 域渗透与横向移动

利用抓取到的凭证，通过psexec等工具结合SMB协议进行横向移动，拿下域控制器。

### 6. 权限维持

生成黄金票据以长期维持对域的控制，添加域管理员账号。

### 7. 痕迹清理

清理系统各类日志（如安全日志、系统日志、应用程序日志等）以掩盖攻击痕迹。

### 8. 红队常用工具使用

如Nmap用于端口扫描，Cobalt Strike用于后渗透（包括上线、提权、横向移动、权限维持等），蚁剑用于连接Webshell等。
